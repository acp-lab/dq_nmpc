...
class DQnmpcNode(Node):
    def __init__(self):
        ...
        self.mav_name = (
            self.get_parameter("mav_name").get_parameter_value().string_value
        )
        ...
        self.Jzz = params["izz"]
        self.J = np.array(
            [[self.Jxx, 0.0, 0.0], [0.0, self.Jyy, 0.0], [0.0, 0.0, self.Jzz]]
        )
        ...
        self.points = None
        self.publisher_ref_trajectory_ = self.create_publisher(
            Marker, "desired_path", 10
        )
        ...
        self.X[:, 0] = np.array(ca.vertcat(self.dual_1, self.dual_twist_1)).reshape(
            (14,)
        )
        ...
        self.dual_twist_1_control = dual_twist(
            self.angular_linear_1_control, self.dual_1_control
        )
    ...
    def callback_get_planner(self, msg):
        ...
        for k in msg.points:
            ...
            self.x_ref[6:10, i] = np.array(
                [k.quaternion.w, k.quaternion.x, k.quaternion.y, k.quaternion.z]
            )
            self.x_ref[10:13, i] = np.array(
                [k.angular_velocity.x, k.angular_velocity.y, k.angular_velocity.z]
            )
            ...
            dual_1_d = dualquat_from_pose(
                qw1_d, qx1_d, qy1_d, qz1_d, tx1_d, ty1_d, tz1_d
            )
            ...
            angular_linear_1_d = np.array(
                [wx_d, wy_d, wz_d, hxd_d, hyd_d, hzd_d]
            )  # Angular Body linear Inertial
            # Init Dual Twist
            ...
            # Desired Torques
            self.w_dot_ref[0:3, i] = np.array(
                [
                    k.angular_velocity_dot.x,
                    k.angular_velocity_dot.y,
                    k.angular_velocity_dot.z,
                ]
            )
